...
export default function Portfolio2() {
  ...
  const importMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const res = await fetch("/api/transactions2/import", { method: "POST", body: form });
      if (!res.ok) {
    ...
    onSuccess: (data) => {
      toast({ title: "帳戶(2)交易明細匯入成功", description: `共匯入 ${data.imported} 筆台股交易` });
      qc.invalidateQueries({ queryKey: ["/api/portfolio2/computed"] });
      qc.invalidateQueries({ queryKey: ["/api/portfolio2-quotes"] });
      refetchHoldings();
    },
    onError: (err: Error) => {
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold">投資組合 (2)</h1>
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-16 gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground/60 mt-1">點擊「更新交易明細」選擇 Excel 檔案，系統將讀取「台股交易明細 (2)」工作表</p>
            </div>
